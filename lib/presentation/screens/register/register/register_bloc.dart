...
import 'package:ecommerce_bloc_app/utils/validator.dart';
...
class RegisterBloc extends Bloc<RegisterEvent, RegisterState> {
  ...
  RegisterBloc() : super(RegisterState.empty()) {
    ...
    on<ConfirmPasswordChanged>(_onConfirmPasswordChanged, transformer: debounce());
  ...
  void _onConfirmPasswordChanged(ConfirmPasswordChanged event, Emitter<RegisterState> emit) {
    var isConfirmPasswordValid = UtilValidators.isValidPassword(event.confirmPassword);
    var isMatched = event.password.isNotEmpty ? event.password == event.confirmPassword : true;
    emit(state.update(isConfirmPasswordValid: isConfirmPasswordValid && isMatched));
  }
